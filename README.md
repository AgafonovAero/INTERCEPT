...
## Физическая конфигурация и первичная ВМГ

PR №8 добавляет контур подготовки физического подтверждения конфигурации изделия и первичной модели винтомоторной группы. Он не выполняет перенос в Simulink.
...
Номерные файлы этапов сохранены как:
...
Физические проверки размещаются локально в `data/raw/physical_checks/`. Стендовые данные ВМГ размещаются локально в `data/raw/bench/propulsion/`. Реальные файлы этих каталогов не добавляются в Git; в репозитории остаются только README и шаблоны.
