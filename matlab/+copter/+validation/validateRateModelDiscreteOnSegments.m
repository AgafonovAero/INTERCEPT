...
for axisIndex = 1:3
    ...
    row.bias_rad_s = metrics.bias(axisIndex);
    row.residual_std_rad_s = metrics.residual_std(axisIndex);
    row.correlation = metrics.correlation(axisIndex);
    row.valid_metric_flag = metrics.valid_metric_flag(axisIndex);
    row.metric_warning = string(metrics.metric_warning(axisIndex));
    rows{axisIndex} = row;
end
end
